...
class BinaryFeatureFunction:
    ...
    def linear_search(self, target, index):
        while index < self.length:
            candidate = self.candidate_list[index]
            if type(candidate) != list: candidate = [candidate]
            if target in candidate: return index
            index += 1
        return -1
...
word_list    = pickle.load(open('/home/ezio/filespace/data/word_list.data', 'rb'))
...
def get_feature(pos_sent, ner_sent, dpt_sent, reverse):
    DEBUG = False
    if DEBUG: print(pos_sent)
    if DEBUG: print(ner_sent)
    if DEBUG: print(dpt_sent)
    #------------------顺逆，长度加 1------------------#
    ...
    if DEBUG: print(len(fea_vec))

    #------------------前中后缀长度作为特征，长度加 1 * 3------------------#
    fea_vec.append(ne1_index) # 前缀长度，长度+1
    fea_vec.append(ne2_index - ne1_index)  # 中缀长度，长度+1
    fea_vec.append(len(ner_list) - 1 - ne2_index) # 后缀长度，长度+1
    if DEBUG: print(len(fea_vec))

    #------------------前中后缀词和词性作为特征，长度加 (34 + 59) * 3------------------#
